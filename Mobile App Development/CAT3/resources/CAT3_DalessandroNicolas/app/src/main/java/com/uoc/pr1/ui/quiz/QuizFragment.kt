...
class QuizFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        // Read the image from the attribute image_path
        if (!item?.image_path.isNullOrEmpty()) {
            val bitmap: Bitmap? = BitmapFactory.decodeFile(item?.image_path)
            bitmap?.let {
                v2.setImageBitmap(it)
            }
        }

        dataSource.selectItemsSeminary(item!!.id)
        if (dataSource.getCountItems() == 0) {
            android.widget.Toast.makeText(context, "Seminary without items", android.widget.Toast.LENGTH_SHORT).show()
